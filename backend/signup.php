...
if($pass == $pass_confirm){
    ...
    $ID = rand(0,1000000);
        $query = 'insert into users(user_id, login, password, avatar, email, phone_number, full_name) values (?,?,?,?,?,?,?)';
        $stmt = $connection->prepare($query);
        $stmt->execute([$ID,$login,md5($pass),$avatar,$email,$phone,$name]);
