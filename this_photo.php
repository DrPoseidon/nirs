...
<body>
<a href="feed.php" style="position: absolute;top:0;left: 0;">Назад</a>
...
<img class = "feed_photos" src="'.$r['Path_to_photo'].'"><div class="down_div" style="height: 61px;border-top: 1px solid black;display: flex;flex-direction: row;align-items: center">';
        ...
        if($s->rowCount() > 0) {
            echo '<a href="backend/remove_like1.php?Ph_ID=' . $r['Photo_ID'] . '&id1='.$r['User_ID'].'"><img src="img/like.png"  style="width: 30px;height: 30px;margin: 10px;"></a>'.$s1->rowCount();
        } else{
            echo '<a href="backend/add_like1.php?Ph_ID=' . $r['Photo_ID'] . '&id1='.$r['User_ID'].'"><img src="img/no-like.png"  style="width: 30px;height: 30px;margin: 10px;"></a>'.$s1->rowCount();
        }
    ...
    <textarea name="comment_text" style="width: 100%;height: 100%;resize: none;padding: 10px;font-size: 20px;"></textarea>
        ...
        <button type="submit" class="comment-s" style="background: initial;border: none;position: relative;top: -50px;left: 92%;"><input type="image" src="img/comment-s.png" style="width: 30px;height: 30px"></button>
<?php
    ...
    foreach ($res_b as $rb) {
        ...
        echo '<textarea name="comment_text" style="width: 75%;height: auto%;resize: none;padding: 10px;font-size: 20px;outline: none;margin-bottom: 10px;" readonly>'.$rb['Comment_text'].'</textarea>
    </div>';
    }
    echo '</div>';
        ?>
